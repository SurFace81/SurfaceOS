...
// Returns only on failure (-1), in which case the old program keeps running.
int      execv(const char* path, char* const argv[]);

// Wait for a child (pid > 0) or any child (pid == -1). *status receives the
// exit status (see EXIT_* in abi/process.h). With WNOHANG returns 0 if no
// child has finished yet. -1 if there is nothing to wait for.
pid_t    waitpid(pid_t pid, int* status, int options);

// Terminate a process of the same session with status EXIT_KILLED.
int      kill(pid_t pid);
