...
static sint32_t control_transfer_in(uint8_t slot_id, uint8_t* setup_packet,
    ...
    if (evt->completion_code != XHCI_TRB_COMPLETION_SUCCESS &&
        ...
        uart::printf("xhci: control IN failed slot=%u code=%u (%s)\n",
                     (uint32_t)slot_id, (uint32_t)evt->completion_code,
...
static bool scsi_inquiry(usb_mass_storage_dev* msd) {
    ...
    for (uint8_t d = 0; d < device_count; d++) {
        if (device_infos[d].slot_id == msd->slot_id) {
            ...
            uart::printf("scsi: vendor='%s' product='%s'\n",
                         device_infos[d].vendor_str, device_infos[d].product_str);
            break;
        }
    }
...
static bool scsi_read_capacity(usb_mass_storage_dev* msd,
    ...
    *out_block_size = ((uint32_t)data[4] << 24) | ((uint32_t)data[5] << 16)
    ...
    uart::printf("scsi: capacity: %u MB, block_size=%u\n",
                 (uint32_t)(total_bytes / (1024 * 1024)), *out_block_size);
...
static void log_cap_regs() {
    uart::printf("xhci: hciversion=%x slots=%u ports=%u scratchpads=%u 64bit=%s\n",
                 (uint32_t)cap_regs->hciversion,
                 (uint32_t)max_device_slots,
                 (uint32_t)max_ports,
                 (uint32_t)max_scratchpad_bufs,
                 XHCI_AC64(cap_regs) ? "yes" : "no");
}
...
static void parse_extended_capabilities() {
    ...
    while (true) {
        ...
        if (cap_id == XHCI_EXT_CAP_PROTOCOL) {
            ...
            uart::printf("xhci: protocol USB%u.%u ports %u-%u\n",
                         (uint32_t)proto.major_rev, (uint32_t)proto.minor_rev,
                         (uint32_t)first_port,
                         (uint32_t)(first_port + port_count - 1));
...
static bool reset_controller() {
    ...
    delay_ms(50);
    uart::printf("xhci: controller reset OK\n");
    return true;
}
...
static bool start_controller() {
    ...
    usbcmd |= XHCI_USBCMD_RUN_STOP | XHCI_USBCMD_INTERRUPTER_ENABLE
            | XHCI_USBCMD_HOSTSYS_ERR_EN;
...
static uint8_t enable_device_slot() {
    ...
    uart::printf("xhci: slot %u enabled\n", (uint32_t)cc->slot_id);
    return cc->slot_id;
}
...
static xhci_input_context* alloc_input_context() {
    return (xhci_input_context*)alloc_xhci_memory(
...
static bool get_device_descriptor(uint8_t slot_id, uint8_t port_speed, uint8_t port_index) {
    ...
    uart::printf("xhci: device slot=%u VID=%x PID=%x USB%x class=%u/%u/%u\n",
                 (uint32_t)slot_id, (uint32_t)desc->idVendor,
                 (uint32_t)desc->idProduct, (uint32_t)desc->bcdUSB,
                 (uint32_t)desc->bDeviceClass, (uint32_t)desc->bDeviceSubClass,
    ...
    while (offset + 2 <= total_len) {
        ...
        if (desc_type == USB_DESC_TYPE_INTERFACE && desc_len >= 9) {
            ...
            if (iface->bInterfaceClass == USB_CLASS_MASS_STORAGE &&
                ...
                uart::printf("xhci: mass storage interface found on slot %u\n",
                             (uint32_t)slot_id);
...
static bool configure_mass_storage(usb_mass_storage_dev* msd) {
    ...
    msd->configured = true;
    uart::printf("xhci: mass storage configured slot=%u\n", (uint32_t)slot_id);
    return true;
}
...
static void setup_device(uint8_t port_index) {
    ...
    if (!create_device_context(slot_id)) return;
    ...
    uart::printf("xhci: device addressed port=%u slot=%u %s\n",
                 (uint32_t)port_index, (uint32_t)slot_id,
...
static usb_status ensure_capacity_cached(uint8_t dev_index) {
    if (dev_index >= mass_storage_count) return USB_ERR_NOT_FOUND;
    if (msd_capacity_cached[dev_index]) return USB_OK;

    usb_mass_storage_dev* msd = &mass_storage_devs[dev_index];
    if (!msd->configured) return USB_ERR_NOT_READY;
...
bool init() {
    ...
    uart::printf("xhci: controller %x:%x at %u:%u.%u\n",
    ...
    for (uint8_t i = 0; i < mass_storage_count; i++) {
        ...
        if (!configure_mass_storage(msd)) continue;
    ...
    uart::printf("usb: init done, %u devices, %u storage\n",
                 (uint32_t)device_count, (uint32_t)mass_storage_count);
    return true;
}
...
usb_status get_device_info(uint8_t index, usb_device_info* out) {
    if (index >= device_count) return USB_ERR_NOT_FOUND;
    if (!out) return USB_ERR_INVALID_PARAM;
...
usb_status get_block_device_info(uint8_t index, usb_block_device* out) {
    if (index >= mass_storage_count) return USB_ERR_NOT_FOUND;
    if (!out) return USB_ERR_INVALID_PARAM;

    usb_status st = ensure_capacity_cached(index);
    if (st != USB_OK) return st;
...
usb_status read_sectors(uint8_t dev_index, uint32_t lba, uint16_t count, void* buffer) {
    if (dev_index >= mass_storage_count) return USB_ERR_NOT_FOUND;
    if (!buffer || count == 0) return USB_ERR_INVALID_PARAM;
